...
RUN set -x \
  ...
  && apt-get install --no-install-recommends --no-install-suggests -y curl nginx gettext libluajit-5.1-2 libluajit-5.1-common libnginx-mod-http-lua libnginx-mod-http-ndk \
...
HEALTHCHECK --interval=5s --timeout=10s CMD curl --silent --fail http://127.0.0.1/ping